...
This prop can be set to make the animations relative to the `<NumberFlow>` itself, rather than the
viewport (default). This can be used to prevent unwanted horizontal animations when the `<NumberFlow>`
component moves around the page.

```tsx
<NumberFlow root />
```

See [the range example](/examples/#range) for a visual demo.
