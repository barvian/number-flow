...
NumberFlow will animate if the following conditions are met:
* The browser [supports composited animations](https://caniuse.com/mdn-api_element_animate_options_composite_parameter)
* The browser [supports `@property` rules](https://caniuse.com/mdn-css_at-rules_property)
* The user has not indicated a preference for reduced motion (this can be bypassed with the [`respectMotionPreference` prop](#respect-motion-preference))
...
### <code>font-variant-numeric<span class="text-muted">:</span> tabular-nums</code>

Ensures all numbers are the same size, which can prevent digits from shifting during transitions.
...
Returns `true` if NumerFlow [can animate](#transitions).
See the [Framer Motion example](/examples#framer-motion) for a usage scenario.


### <code>useLinear<Type>{'<T>'}</Type>(linearBased<Type>: T</Type>, fallback<Type>: T</Type>)<Type>: T</Type></code>

An SSR-safe way to use a `linear()`-based value if the browser supports it, and a fallback value
otherwise:

```ts
// Use linear() timing if supported, otherwise cubic-bezier():
...
* Some locales that use non-English characters (e.g. `fa-IR`, `ar-OM`) [aren't supported yet](https://github.com/barvian/number-flow/issues/8), but will be soon
