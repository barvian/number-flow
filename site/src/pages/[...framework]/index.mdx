...
```jsx
// Basic usage
// Ensure your project is set up for React Native
import NumberFlow from 'number-flow-react-native'
import { View } from 'react-native';

// <NumberFlow value={123} />
// Example in a component:
const MyComponent = () => <View><NumberFlow value={123} /></View>;
```
</Match>
<Match vue>
```vue
...
<Match react_native>
```jsx
// Note: Animations are not fully supported in React Native yet.
// These props are available but may not have the same effect.
<NumberFlow
	// Used for layout-related transforms:
	transformTiming={{ duration: 750, easing: 'linear(...)' }}
	// Used for the digit spin animations.
...
<Match react_native>
```jsx
// Note: Plugins might have limited or no effect in React Native yet.
import NumberFlow from 'number-flow-react-native'
// import { continuous } from 'number-flow/lite'; // or from 'number-flow' if re-exported

// <NumberFlow
//	 plugins={[continuous]} // Assuming continuous is adapted or available
//	 value={value}
// />
```
</Match>
...
<Match react_native>
```js
// Styling in React Native is done via StyleSheet or inline styles.
// The `style` and `textStyle` props can be used.
// Example:
// <NumberFlow value={123} style={{backgroundColor: 'blue'}} textStyle={{color: 'white'}} />

// The concept of CSS `::part` does not directly map.
// Custom styling would involve passing props to affect internal Text/View components.
// For advanced styling, the component might need to expose more specific style props.
// Note: Current RN component uses basic View and Text. No direct ::part equivalent.
```
</Match>
<Match vue>
```css
...
<code slot="react_native">// React Native's Animated API or Reanimated would be used for animations.
// `AccessibilityInfo.isReduceMotionEnabled()` can check motion preference.
// No direct `useCanAnimate` hook, behavior depends on chosen animation library.</code>
...
<Match react_native>
In React Native, animation capabilities are provided by the platform's native animation APIs (like `Animated`) or libraries such as `Reanimated`.
The `respectMotionPreference` prop in the component should ideally use `AccessibilityInfo.isReduceMotionEnabled()` to determine if animations should be reduced or disabled.
There isn't a direct equivalent to `useCanAnimate` as it's more about the animation library's capabilities and the device's settings.
</Match>
<Match vue>
