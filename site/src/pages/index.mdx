...
<MotionNumber
    value={value}
    format={{ notation: 'compact' }} // Intl.NumberFormat() options parameter
    locales="en-US"
/>
```

For a full list of `locales` and `format` options, [see MDN's reference for `Intl.NumberFormat()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat/NumberFormat#parameters).
...
There's also some CSS properties you can use to customize the display:
...
In my opinion, the number transitions look best with short gaps between numbers to reduce
the distance they travel. I recommend using the smallest `line-height` that fits all your component's characters:
