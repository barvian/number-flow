...
title: Number Flow
description: Transition, format, and localize numbers. Accessible and customizable.
---
...
Number Flow accepts a `timings` prop to customize the transitions. See [MDN's `KeyframeEffect` reference](https://developer.mozilla.org/en-US/docs/Web/API/KeyframeEffect/KeyframeEffect#options) for a full list of options.
...
<NumberFlow
    timings={{
        x: { duration: 700, easing: 'ease-out' },
        y: { duration: 700, easing: 'ease-out' },
        opacity: { duration: 300, easing: 'ease' }
    }}
/>
```
{/* </DemoTransition> */}
